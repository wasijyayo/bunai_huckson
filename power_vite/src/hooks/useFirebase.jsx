...
const useFirebase = () => {
  ...
  useEffect(() => {
    const unsubscribeAuth = auth.onAuthStateChanged((user) => {
      ...
      if (user) {
        setEmail(user.email);
        fetchDb(user.email); // ✅ learningsに保存
  ...
  const fetchDb = async (email) => {
    ...
    try {
      const url =
        "https://asia-northeast1-power-bunai.cloudfunctions.net/getCollection?collection=" +
        email;
  ...
  const editDb = async (editLearning) => {
    try {
      if (!editLearning.id) {
        ...
        await fetch(
          "https://asia-northeast1-power-bunai.cloudfunctions.net/updateLearning",
          {
            method: "POST",
  ...
  const GeminiTestasync = async (sendText) => {
    fetch(
      "https://asia-northeast1-power-bunai.cloudfunctions.net/getGimini?contents=" +
